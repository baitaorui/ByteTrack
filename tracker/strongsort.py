...
class StrongSORT(BaseTracker):
    def __init__(self, opts, frame_rate=30, gamma=0.1, use_ECC=True, use_AFLink=True, use_GSI=True, num_of_budget=20, 
        ...
        load_pretrained_weights(self.reid_model, '/home/btr/ByteTrack/tracker/reid-weights/osnet_x0_25.pth')
        ...
        self.matching_thresh = min(0.3, self.opts.match_thresh - 0.2)  # matching thresh
    ...
    def update(self, det_results, ori_img):
        ...
        scores_t = det_results[:, 4]
        bboxes_t = det_results[:, :4]
        scores = []
        bboxes = []
        for i in range(0, len(bboxes_t)):
            ...
            for ii in bboxes_t[i]:
                if ii < 0:
                    flag = True
            if not flag:
                bboxes.append(bboxes_t[i])
                scores.append(scores_t[i])
        ...
        if det_results.shape[0] > 0:
            features = self.get_feature(bboxes, ori_img)
            detections = [STrack(cls, tlbr_to_tlwh(xyxy), score, feature=feature, kalman_format='strongsort')
                            for (cls, xyxy, score, feature) in zip([0 for _ in bboxes], bboxes, scores, features)]
        else:
            detections = []
