...
class UAVMOT(BaseTracker):
    def __init__(self, opts, frame_rate=30, gamma=0.1, *args, **kwargs) -> None:
        ...
        self.use_apperance_model = True  
        self.reid_model = Extractor('/home/btr/ByteTrack/tracker/reid-weights/ckpt.t7', use_cuda=True)
        self.gamma = gamma  # coef that balance the apperance and ious

        self.low_conf_thresh = max(0.15, self.opts.track_thresh - 0.3)  # low threshold for second matching
    ...
    def update(self, det_results, ori_img):
        ...
        bboxes_t = bboxes[det_high_indicies]
        bboxes_t2 = bboxes[det_low_indicies]
        scores_t = scores[det_high_indicies]
        scores_t2 = scores[det_low_indicies]
        bboxes_high = []
        scores_high = []
        bboxes_low = []
        scores_low = []
        for i in range(0, len(bboxes_t)):
            flag = False
            for ii in bboxes_t[i]:
                if ii < 0:
                    flag = True
            if not flag:
                bboxes_high.append(bboxes_t[i])
                scores_high.append(scores_t[i])
        for i in range(0, len(bboxes_t2)):
            flag = False
            for ii in bboxes_t2[i]:
                if ii < 0:
                    flag = True
            if not flag:
                bboxes_low.append(bboxes_t2[i])
                scores_low.append(scores_t2[i])        
        ...
        if det_high.shape[0] > 0:
            if self.use_apperance_model:
                ...
                features = self.get_feature(bboxes_high, ori_img)
                D_high = [AMF_STrack(cls, tlbr_to_tlwh(xyxy), score, feature=feature)
                            for (cls, xyxy, score, feature) in zip([0 for _ in bboxes], bboxes_high, scores_high, features)]
            else:
                ...
                D_high = [AMF_STrack(cls, tlbr_to_tlwh(xyxy), score)
                            for (cls, xyxy, score) in zip([0 for _ in bboxes_high], bboxes_high, scores_high)]
        else:
            D_high = []

        if det_low.shape[0] > 0:
            features = self.get_feature(bboxes_low, ori_img)
            D_low = [AMF_STrack(cls, tlbr_to_tlwh(xyxy), score, feature=feature)
                            for (cls, xyxy, score, feature) in zip([0 for _ in bboxes_low], bboxes_low, scores_low, features)]
        else:
            D_low = []
