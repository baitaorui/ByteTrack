...
from .kalman_filter import BoTSORTKalmanFilter
...
class BoTSORT(BaseTracker):
    def __init__(self, opts, frame_rate=30, gamma=0.02, use_GMC=False, *args, **kwargs) -> None:
        ...
        self.kalman = BoTSORTKalmanFilter()
    ...
    def update(self, det_results, ori_img):
        ...
        if det_high.shape[0] > 0:
            D_high = [STrack(cls, tlbr_to_tlwh(xyxy), score, kalman_format='botsort')
                            for (cls, xyxy, score) in zip([0 for _ in bboxes_high], bboxes_high, scores_high)]
        else:
            D_high = []

        if det_low.shape[0] > 0:
            D_low = [STrack(cls, tlbr_to_tlwh(xyxy), score, kalman_format='botsort')
