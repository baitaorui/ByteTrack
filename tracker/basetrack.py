...
class STrack(BaseTrack):
    ...
    def tlwh(self):
        ...
        if self.kalman_format in ['default', 'strongsort', 'new']:
...
class BaseTracker(object):
    def __init__(self, opts, frame_rate=30, *args, **kwargs) -> None:
        ...
        self.frame_id = 0
        self.det_thresh = opts.track_thresh
    ...
    def update(self, det_results, ori_img):
        ...
        if det_results.shape[0] > 0:
            detections = [STrack(cls, tlbr_to_tlwh(xyxy), score)
                            for (cls, xyxy, score) in zip([0 for _ in bboxes], bboxes, scores)]
        else:
            detections = []
        ...
        matched_pair, u_track, u_detection = matching.linear_assignment(IoU_mat, thresh=0.6)
        ...
        matched_pair1, u_track1, u_detection1 = matching.linear_assignment(IoU_mat, 0.7)
